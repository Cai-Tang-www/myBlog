...
这篇不是想讲“SubAgent 有多酷”，也不是泛泛介绍多智能体架构。相反，它更像一次项目复盘：我一开始确实想把 SubAgent、Todo、Scheduler、DAG 都做起来，但后来在会议讨论和实际排障里发现，当前阶段最重要的不是先把并发编排做复杂，而是先把**单 Agent 主链路闭环**和**可控的 inline 子代理**做稳。
...
所以这篇文章的主线是：**SubAgent 不是并发炫技，而是复杂任务治理。**

## 1. 先讲用户场景：为什么用户需要 SubAgent
...
## 2. 先定义概念：ReAct、SubAgent、inline、DAG、Coordinator
...
## 3. 为什么 NeoCode 当前选择 inline 最小实现
...
## 6. 当前实现链路：spawn_subagent(mode=inline) 怎么跑
...
> schema 暗示一个不存在的能力，比缺能力更危险。因为模型会认真相信 schema。
...
## 10. issue / 里程碑视角：风险、返工和应对

团队开发里，每个 issue 不只是“功能点”，还要回答：用户价值是什么？截止前风险是什么？是否可能返工？依赖谁？

我现在会这样看 SubAgent 相关模块：
...
这张表对我最大的帮助是：它强迫我承认“当前能交付什么”，而不是把未来愿景写成当前能力。

当前里程碑的目标不是做完整多智能体平台，而是交付一个可信的 inline 子代理闭环。DAG / Coordinator 是后续演进，不应该在截止前强行塞进同一个 issue。

## 11. 下一阶段：从 inline 到可控并发
...
- `max_concurrent = 1 / 2`；
- 支持 timeout；
- 支持 cancel propagation；
- 支持失败分类；
- 不表达复杂依赖图。

这一步只解决“有限并发”，不解决复杂 DAG。

### Phase 3：DAG / Coordinator

进入这一阶段必须有前提：

- inline 稳；
- queue 稳；
- Git / workspace change tracking 可用；
- result merge policy 明确；
- conflict detection 可用；
- 任务审计链路完整。

DAG 是后期编排形态，不是第一版实现手段。

## 12. 结尾：我从这条线学到的工程方法

这次最大的变化是，我不再把“能做出来”当唯一标准。
...
之前开会强调“Agent 的价值来自模型之外的判定与反馈能力”，这句话也改变了我对 SubAgent 的理解。SubAgent 不是主循环之外的另一个黑盒模型，而应该是主循环闭环中的一个受控执行单元。它要服务于任务拆解、工具协作、验证反馈和最终完成判定。
...
> SubAgent 不是并发炫技，而是复杂任务治理。先把 inline 做成可信执行单元，再谈 DAG 和 Coordinator，才是对用户、团队和项目进度都更稳的路线。也希望能给屏幕前的你带来些许启发。
