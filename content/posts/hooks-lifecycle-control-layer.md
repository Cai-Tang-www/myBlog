...
我现在更偏向于把它理解成一层 **middleware / extension layer**。不是新的业务模块，不是新的控制平面，而是在 runtime 执行过程中，为关键节点暴露一组扩展接口。
...
一套成熟的 hooks 系统，不能让 hook 的输出只是一段随意字符串。它需要一套**结构化响应协议**。
...
hooks 的难点其实不在“它能做什么”，而在“它做到哪一步才不把系统搞危险”。
